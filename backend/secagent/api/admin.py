...
from secagent.services.provider_routes import (
...
class ProviderRouteRead(BaseModel):
    provider: str
    route: str
    display_name: str
    base_url: str
...
class ProviderRouteWrite(BaseModel):
    model_config = ConfigDict(extra="forbid")

    route: Literal["opencode-go", "deepseek"]
...
def _route_read(status: ProviderRouteStatus) -> ProviderRouteRead:
    return ProviderRouteRead(
        provider=status.provider,
        route=status.route,
...
@router.get("/provider-routes/deepseek/options")
def deepseek_route_options(actor: AdminDep) -> list[dict]:
    del actor
    return list(DEEPSEEK_ROUTE_PRESETS.values())


@router.get("/provider-routes/deepseek", response_model=ProviderRouteRead)
def get_deepseek_route(request: Request, actor: AdminDep) -> ProviderRouteRead:
    del actor
    with request.app.state.session_factory() as session:
        status = ProviderRouteStore(
            session, request.app.state.settings
        ).deepseek_status()
    return _route_read(status)


@router.put("/provider-routes/deepseek", response_model=ProviderRouteRead)
def save_deepseek_route(
    payload: ProviderRouteWrite, request: Request, actor: AdminDep
) -> ProviderRouteRead:
    with request.app.state.session_factory() as session:
        status = ProviderRouteStore(
            session, request.app.state.settings
        ).save_deepseek_route(payload.route, actor.id)
    return _route_read(status)
