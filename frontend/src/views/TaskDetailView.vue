...
const canCancel = computed(() =>
  ['created', 'running', 'waiting_human', 'paused', 'failed_retryable'].includes(task.value?.status || ''),
)

async function refresh() {
  await store.startPolling(taskId)
...
async function decide(approved: boolean, reason: string) {
  busy.value = true
  try {
    await lifecycle.approve(taskId, approved, reason)
  ...
  <section class="page" v-if="task">
    <header class="task-hero panel">
      <div>
        ...
        <h1>{{ task.goal }}</h1>
        <p>{{ task.authorization_scope }}</p>
      </div>
      <div class="task-summary">
        <span class="status-badge" :data-status="task.status">{{ task.status }}</span>
        <span>{{ task.scene || task.scene_hint || '场景待识别' }}</span>
        <span>{{ task.route_mode === 'auto' ? 'GLM + DeepSeek 自动协同' : task.preferred_model }}</span>
    ...
    <div class="task-actions">
      ...
      <button v-if="task.pending_approval" class="warning-button" @click="approvalOpen = true">查看待审批动作</button>
    ...
    <div class="detail-grid">
      <section class="panel detail-column">
        <header class="panel-title"><div><p class="eyebrow">DECISION TRACE</p><h2>执行时间线</h2></div><span>{{ task.steps.length }} 步</span></header>
        <StepTimeline :steps="task.steps" :is-demo="task.is_demo" />
      </section>
      <aside class="panel evidence-column">
        ...
        <div v-else-if="tab === 'tools'" class="tool-call-list">
          <article v-for="call in task.tool_calls" :key="call.id"><header><code>{{ call.tool_name }}</code><span>{{ call.status }}</span></header><pre>{{ JSON.stringify(call.result, null, 2) }}</pre></article>
